...
//            lines.forEach(c -> System.out.println(c));
            ...
            for ( String line : lines
                ...
                i++;
                String regex = "G0[12]"; // регулярное выражение
            ...
            // вывод граничных номеров линий кода формирования детали
            System.out.println(frameNumbers.get(0)+" "+frameNumbers.get(frameNumbers.size()-1));
            ...
            for ( String line : lines
                ...
                i++;
                if(frameNumbers.get(0)<=i && frameNumbers.get(frameNumbers.size()-1)>=i) {
                    partManufacturingCode.add(line);
                    partManufacturingCode.add("\n");
                }
            }

            //вывод кода формирования детали
           // System.out.println(partManufacturingCode);
            ...
            border = sc.nextInt();
            //System.out.println(quantityByX+"  "+quantityByY+" "+border);
            sc.close();

            // формирование кода набора деталей по Х
            List<String> partManufacturingCodeX = new ArrayList<>();
            List<String> partManufacturingCodeY = new ArrayList<>();
            ...
            writer.write(String.valueOf(partManufacturingCodeXY));
            writer.flush();
            writer.close();
        } catch (IOException e) {
    ...
    public static List<String> duplicationAlongGivenAxis (int quantityBy, int borderP, float size, String a,
                                                          String regexFor,
     List<String> partManufacturingCodeP){
        List<String> partManufacturingCode = new ArrayList<>();
        for (int j = 0; j < quantityBy; j++) {
            ...
            for (String part : partManufacturingCodeP
                ...
                while (matcherFor.find()) { // возврящает true когда найдено соответствие
                    ...
                    String correctedPart = part.replaceAll(sourceString, a + correctedString);
                    partManufacturingCode.add(correctedPart.concat("\n"));
                    // заданного интревала
                }
            }
        }
